...
csv_file = csv_file_path.__str__() + '/Monash_PG_unordered.csv'
...
for each_url in course_links_file:
    ...
    anotherName = soup.find('h1', class_="headline-60px inverted-headline")
    if courseName:
        course_level = courseName.find('small').text
        course_title = courseName.text.replace(course_level, "")
        course_data['Course'] = course_title
    elif anotherName:
        course_level = anotherName.find('small').text
        course_title = anotherName.text.replace(course_level, "")
        course_data['Course'] = course_title
    else:
        course_level = ''
        courseName = ''
    ...
    try:
        ti = soup.select('.inner .hidden div:nth-of-type(2) div:nth-of-type(1) p')
        if ti:
...
ordered_file = csv_file_path.parent.__str__() + "/UNC_PG_ordered.csv"
