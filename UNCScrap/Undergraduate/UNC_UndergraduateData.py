...
csv_file = csv_file_path.parent.__str__() + '/UNC_UG_undergraduate.csv'
...
def durations(duration_location):
    try:
        p_word = duration_location.text.strip()
...
for each_url in course_links_file:
    ...
    # Description
    decrip = soup.select('.body-content > div.grid-2-column div.grid-block:nth-of-type(1)')
    degree_details = soup.select("#degree-details > p:nth-child(4)")
    if decrip:
        description(decrip)
    elif degree_details:
        description(degree_details)
    else:
    ...
    # Availability Part 1
    header_1 = soup.select_one(".inner div.fast-facts-header")
    if header_1:
        if "available for international students only" in header_1.text.lower():
            course_data['Availability'] = "I"

    # Cities
    cities = soup.select(".inner .fast-fact-toggle a")
    city_ = soup.select("#panel > div.uon-snapshot.uon-snapshot-ug > div > div.uon-form-selector > select")
    if cities:
        for a in cities:
    ...
    try:
        ...
        money2 = soup.select(
            "#panel > div.fast-facts.ff-in-header.fast-facts-ug > div > div.fast-facts-content > "
            "div.fast-fact-items.visible > div:nth-child(2) > div:nth-child(1) > p")
    ...
    # Duration
    ti = soup.select("tr:contains('Duration') td")
    if ti:
        for time1 in ti:
    ...
    try:
        ...
        if button:
            ...
            course4 = browser.find_elements_by_css_selector(".course-requirement-group .title a")
            course3 = browser.find_elements_by_css_selector("#section-core-courses-directed .title a")
            if course2:
                for to in course2:
    ...
    for each_link in courses_detail:
        ...
        if course_desc:
            ...
            course_data[f'Subject_Objective_{i}'] = "N/A"
