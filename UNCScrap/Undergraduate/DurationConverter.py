...
def convert_num(text):  # okay... but why tho? what about zero?
    return text.replace('one', '1').replace('two', '2').replace('three', '3').replace('four', '4'). \
    ...
    for number in nums:
        ...
        for dur in dur_type_list:
            ...
            # CHECK FOR "MONTH"
                ...
                return convert_duration(str(int(number) * 6) + 'months')
            elif 'trimester' in dur:
                return convert_duration(str(int(number) * 3) + 'months')
            elif 'term' in dur:
                return convert_duration(str(int(number) * 6) + 'months')
            elif 'day' in dur:
                if '.' in number:
