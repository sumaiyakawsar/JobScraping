...
csv_file = csv_file_path.parent.__str__() + '/QUT_all_courses.csv'
...
for each_url in course_links_file:
    course_data = {'Level_Code': '', 'University': 'Queensland University of Technology', 'City': '',
                   'Course': '', 'Faculty': '',
                   'Int_Fees': '', 'Local_Fees': '', 'Currency': 'AUD', 'Currency_Time': 'Years',
                   'Duration': '', 'Duration_Time': '', 'Full_Time': '', 'Part_Time': '',
                   'Prerequisite_1': 'ATAR', 'Prerequisite_2': 'IELTS', 'Prerequisite_3': 'Equivalent ',
    ...
    course_name = soup.select(
        "body > div.header__wrapper > div.hero__header.course__page > div.hero__header__container.container > div > "
        "h1 > span")
    if course_name:
        for course in course_name:
            course_title = tag_text(course)
            course_data['Course'] = course_title
    if "bachelor" in course_data['Course'].lower() and "honours" in course_data['Course'].lower():
    ...
    print(course_data['Course'],course_data['Level_Code'])
    # city/study mode
    ...
    if 'Sydney' in actual_cities or 'Canberra' in actual_cities or 'Brisbane' in actual_cities:
    ...
    if course_highlights:
        for each_desc in course_highlights:
            describe.append(each_desc.text.replace("\n", " "))
    if courseDescription0:
        for each_desc in courseDescription0:
            describe.append(each_desc.text.replace("\n", " "))
    if courseDescription1:
        for each_desc in courseDescription1:
            describe.append(each_desc.text.replace("\n", " "))
    if courseDescription2:
        for each_desc in courseDescription2:
            describe.append(each_desc.text.replace("\n", " "))
    ...
    # career Outcome
    care = []
    career = soup.select(".course-possible-careers li")
    if career:
        for each_career in career:
            care.append(each_career.text.strip())
    course_data['Career_Outcomes/path'] = ', '.join(care)
