...
csv_file = csv_file_path.parent.__str__() + '/Monash_PG_courses.csv'
...
for each_url in course_links_file:
    course_data = {'Level_Code': '', 'University': 'Monash University', 'City': '', 'Course': '', 'Faculty': '',
                   ...
                   'Website': '', 'Course_Lang': 'English', 'Availability': 'A', 'Description': '', 'Country': '',
    ...
    if divDescription:
        course_description = divDescription.find_all("p")
        if course_description:
            for ro in course_description:
                description_1 = ro.text
                description.append(description_1)

    divdescription2 = soup.select(".slider-tab-content p")
    if divdescription2:
        for each in divdescription2:
            description_2 = each.text.strip()
    ...
    if div1:
        div2 = div1.find('ul')
        if div2:
            on_off = div2.text.strip().lower()
            if 'off-campus' in on_off:
            ...
            if 'full time' in on_off:
                course_data['Full_Time'] = 'Yes'
            else:
                course_data['Full_Time'] = 'No'
            if 'part time' in on_off:
                course_data['Part_Time'] = 'Yes'
            else:
                course_data['Part_Time'] = 'No'
            ...
            for each_line in li_all:
                tempLine = each_line.text.lower().strip()
