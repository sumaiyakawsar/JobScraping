...
course_links_file_path = course_links_file_path.__str__() + '/Monash_graduate_links.txt'
course_links_file = open(course_links_file_path, 'r')

# the csv file we'll be saving the courses to
csv_file_path = Path(os.getcwd().replace('\\', '/'))
csv_file = csv_file_path.__str__() + '/Monash_grad.csv'
...
for each_url in course_links_file:
    ...
    # COURSE TITLE & Level
    div_left = soup.find('div', class_='grid-col-sm-12 grid-col-md-12-12 page-header-courses')
    if div_left:
        course_title = soup.find('strong', class_='h1').text

        course_level = div_left.p.text
    ...
    # print(course_data['Course'], course_data['Level_Code'],course_data['Course_Level'],course_data['Faculty'], )

    # Course_Description
    divDescription = soup.find('div', class_="course-page__summary-text")
    if divDescription:
        course_description = divDescription.p.text
    ...
    if div1:
        ...
        if div2:
            ...
            for each_line in li_all:
                ...
                if 'full time' in tempLine.lower() and 'part time' not in tempLine.lower():
                    course_data['Full_Time'] = 'Yes'
                else:
                    course_data['Full_Time'] = 'No'
                if 'part time' in tempLine.lower() or 'part' in tempLine.lower() and 'full time' not in tempLine.lower():
                    course_data['Part_Time'] = 'Yes'
                else:
                    course_data['Part_Time'] = 'No'
                #print(tempLine)
                if 'full time' in tempLine.lower() and 'part time' in tempLine.lower():
                    course_data['Blended'] = 'Yes'
                else:
                    course_data['Blended'] = 'No'
                if 'off-campus' in tempLine.lower():
    ...
    #print(course_data['Website'], course_data['Full_Time'], course_data['Part_Time'], course_data['Blended'])
...
ordered_file = csv_file_path.parent.__str__() + '/Monash_grad_ordered.csv'
