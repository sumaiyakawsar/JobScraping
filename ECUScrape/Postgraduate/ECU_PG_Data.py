...
csv_file = csv_file_path.parent.__str__() + '/ECU_PG_Degrees.csv'
...
def duration(p_word):
    try:
        if p_word:
            if 'full-time' in p_word.lower() or 'full time' in p_word.lower():
                course_data['Full_Time'] = 'Yes'
            else:
                course_data['Full_Time'] = 'No'

            if 'part-time' in p_word.lower() or 'part time' in p_word.lower():
...
for each_url in course_links_file:
    ...
    if 'honours' in course_data['Course'].lower() and 'bachelor' in course_data['Course'].lower():
    ...
    if courseDescription:
        description.append(courseDescription.text.strip().replace("\n", " "))
    else:
        for ea in courseDesc2:
            description.append(ea.text.strip().replace("\n", " "))
    course_data['Description'] = ' '.join(description)

    # local fees
    try:
        localmoney = soup.select(
            "#overview > div.quickReference.audience__panel--domestic > div:nth-child(4) > p:nth-child(2) > span > "
            "strong")
    ...
    try:
        moneyColumn = soup.select(
            "#overview > div.quickReference.audience__panel--international > div:nth-child(4) > p:nth-child(2) > span "
            "> strong")
        ...
        if moneyColumn:
            ...
            international_fee = i_fees(Int_money)
            course_data['Int_Fees'] = international_fee
        elif more_int:
            international_fee = i_fees(more_int)
            course_data['Int_Fees'] = international_fee
    ...
    # Duration/Duration Time/Full-Time/Part-time/
    ...
    dura_detail = soup.select(".progdetails li:nth-of-type(1)")
    dura_detail2 = soup.select(".audience__panel--domestic div:nth-of-type(5) p:nth-of-type(1)")
    if course_detail:
        ...
        for so in dura_detail:
            p_word = so.text.strip()
            duration(p_word)
    if dura_detail2:
        for to in dura_detail2:
