...
csv_file = csv_file_path.parent.__str__() + '/CDU_PG_allCourses.csv'
...
for each_url in course_links_file:
    ...
    if "D" in course_data['Availability']:
        domestic_ = soup.select("div[data-student-type='domestic']:nth-of-type(3) div.fable__value")
        if domestic_:
            durations(domestic_)
    ...
    more_links = soup.select("div:nth-of-type(3) td a")
    if all_links:
    ...
    for each_links in subject_links:
        ...
        learning_outcomes = unit_soup.select_one(".LearningOutcomes")
        if learning_outcomes:
            outcomes = learning_outcomes.text.replace("\n", "").replace(r'/[^\x00-\x7F]/g', "").strip()
            if outcomes is not '':
